          ...
          <div class="card flex justify-content-between"> 
              ...
              <div class="flex flex-column"> 
                ...
                <select v-model="selectedCourse" class="w-full md:w-17rem p-2 bg-pink-300 border-0 outline-none text-lg cursor-pointer"> 
                  <option class="text-lg" :value="null">All Courses</option> 
          ...
          <div  class="grid" v-if="selectedYear !== null && selectedMonth !== null"> 
            <div class="col-12"> 
                ...
                <table class="w-full"> 
                  ...
                  <tbody> 
                    <tr v-for="course in filteredCourses(filteredLectures)" :key="course"> 
                      <td class="pl-2">{{ course }}</td> 
                      <td v-for="day in getDaysInMonth(selectedYear, selectedMonth)" :key="day" 
                        :class="getBackgroundColorClassForDate(day, filteredLectures.filter(cell => cell.course.name === course))" 
            ...
            <div class="col-12"> 
                ...
                <table class="w-full"> 
                  <thead> 
                    <tr> 
                      <th>Course</th> 
                      <th v-for="day in getDaysInMonth(selectedYear, selectedMonth)" :key="day" :class="{ 'bg-pink-300': isWeekend(day) }"> 
                        {{ day }} 
                      </th> 
                    </tr> 
                  </thead> 
                  <tbody> 
                    <tr v-for="course in filteredCourses(filteredPractices)" :key="course"> 
                      <td class="pl-2">{{ course }}</td> 
                      <td v-for="day in getDaysInMonth(selectedYear, selectedMonth)" :key="day" 
                        :class="getBackgroundColorClassForDate(day, filteredPractices.filter(cell => cell.course.name === course))" 
                        style="width: 30px; height: 30px"></td> 
                    </tr> 
                  </tbody> 
                </table> 
